 ...
 *
 */
public class UnsignedByteImageConverter implements ImageConverter {
    ...
    public void convertTo(Object arr, double[] target)
    {
        ...
        {
            throw new RuntimeException("The arrays passed to the UnsignedByteImageConverter are not of the same size.");
        }
        for(int i = 0; i < larr.length;i++)
        {
            target[i] = (((double)(larr[i] & 0xFF))/255.0);
        }
    }
    ...
    public void convertTo(final Object arr, final DoubleBuffer target)
    {
        ...
        for(int i = 0; i < larr.length;i++)
        {
            target.put((((double)(larr[i] & 0xFF))/255.0));
        }
        target.rewind();
    }
    ...
    public void deConvertTo(double[] target, Object arr) {
        ...
        for(int i = 0; i < larr.length;i++)
        {
            double currentValue = target[i] * 255.0;
            if(currentValue < 0.0)
            {
                larr[i] = 0;
            }
            ...
            {
                larr[i] = (byte)(target[i] + 0.5); // because target[i] is never negative this is rounding by truncation
            }
        }
    }
    ...
    public void deConvertTo(DoubleBuffer target, Object arr) {
        ...
        for(int i = 0; i < larr.length;i++)
        {
            double currentValue = target.get(i) * 255.0;
            if(currentValue < 0.0)
            {
                larr[i] = 0;
            }
            ...
            {
                larr[i] = (byte)(currentValue + 0.5);// because currentValue is never negative this is rounding by truncation
            }
        }
        target.rewind();
    }
    ...
    public void convertTo(Object arr, float[] target) {
        ...
        for(int i = 0; i < larr.length;i++)
        {
            target[i] = (((float)(larr[i] & 0xFF))/255.0f);
        }
    }
    ...
    public void deConvertTo(float[] target, Object arr) {
        ...
        for(int i = 0; i < larr.length;i++)
        {
            float currentValue = target[i] * 255.0f;
            if(currentValue < 0.0f)
            {
                larr[i] = 0;
            }
            else if(currentValue > 255.0f)
            {
                larr[i] = (byte)-1;
            }
            else
            {
                larr[i] = (byte)(currentValue + 0.5f); // because target[i] is never negative this is rounding by truncation
            }
        }
    }
}
