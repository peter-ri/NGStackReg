 ...
 *
 */
public class LongImageConverter implements ImageConverter {
    @Override
    public void convertTo(final Object arr, final double[] target)
    {
        ...
        {
            throw new RuntimeException("The arrays passed to the LongImageConverter are not of the same size.");
        }
        for(int i = 0; i < larr.length;i++)
        {
            target[i] = ((double)larr[i]/((double)Long.MAX_VALUE));
        }
    }
    @Override
    public void convertTo(final Object arr, final DoubleBuffer target)
    {
        ...
        for(int i = 0; i < larr.length;i++)
        {
            target.put(((double)larr[i]/((double)Long.MAX_VALUE)));
        }
        target.rewind();
    }
    ...
    public void deConvertTo(double[] target, Object arr) {
        ...
        for(int i = 0; i < larr.length;i++)
        {
            double currentValue = target[i] * ((double)Long.MAX_VALUE);
            if(currentValue < ((double)Long.MIN_VALUE))
            {
                larr[i] = Long.MIN_VALUE;
            }
            else if(currentValue > ((double)Long.MAX_VALUE))
            {
                larr[i] = Long.MAX_VALUE;
            }
            else
            {
                larr[i] = (long)Math.round(currentValue);
            }
        }
    }
    ...
    public void deConvertTo(DoubleBuffer target, Object arr) {
        ...
        for(int i = 0; i < larr.length;i++)
        {
            double currentValue = target.get(i) * ((double)Long.MAX_VALUE);
            if(currentValue < ((double)Long.MIN_VALUE))
            {
                larr[i] = Long.MIN_VALUE;
            }
